...
def test_extract_parameters():
    from _inputs import _extract_parameters
    input_params = {
        "sizeoffwhm": 30,
...
def test_read_and_check_tsv_file(tmpdir):
    ...
    with pytest.raises(
        ...
        _read_and_check_tsv_file(tmpdir / "foo.tsv")
    df = _read_and_check_tsv_file(Path(CURRENT_DIR).parent / "data/subjects.tsv")
    assert len(df) == 7
...
def test_get_t1_freesurfer_custom_file_template():
    ...
    assert (
        path == (
            "./sub-01/ses-M00/t1/freesurfer_cross_sectional/"
            "sub-01_ses-M00/surf/lh.thickness.fwhm20.fsaverage.mgh"
        )
    )


def test_check_contrast():
    from _inputs import _check_contrast, _read_and_check_tsv_file
    df_subjects = _read_and_check_tsv_file(Path(CURRENT_DIR).parent / "data/subjects.tsv")
    with pytest.raises(
        ValueError,
